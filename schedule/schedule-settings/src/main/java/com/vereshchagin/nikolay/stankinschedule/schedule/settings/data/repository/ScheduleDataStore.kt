package com.vereshchagin.nikolay.stankinschedule.schedule.settings.data.repository
...
class ScheduleDataStore @Inject constructor(
    ...
    private fun Preferences.colorOrDefault(type: PairColorType): String {
        return this[keyForColorType(type)] ?: type.hex
    }

    companion object {

        private fun keyForColorType(type: PairColorType): Preferences.Key<String> {
            return when (type) {
        ...
        private val FAVORITE_SCHEDULE_ID get() = longPreferencesKey("favorite_schedule_id")

        private val VERTIVAL_VIEWER get() = booleanPreferencesKey("schedule_vertical_viewer")
    }
}