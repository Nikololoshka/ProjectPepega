...
class NewsRemoteRepository2024Impl @Inject constructor(
    ...
    override suspend fun loadPage(
        ...
        val newsBlock = Regex("""<a.+?class="newsItem".*?>.+?</a>""", RegexOption.DOT_MATCHES_ALL)
        ...
        return newsBlock.findAll(text)
            ...
            .map { block ->
                NewsPost(
                    id = 0,
                    title = newsTitle.find(block).getOrThrow(1),
                    previewImageUrl = StankinNews2024API.BASE_URL + newsImage.find(block)
                        .getOrThrow(1),
                    date = processDate(newsDate.find(block).getOrThrow(1)),
                    relativeUrl = StankinNews2024API.BASE_URL + newsLink.find(block).getOrThrow(1),
                )
            }
            .toList()
